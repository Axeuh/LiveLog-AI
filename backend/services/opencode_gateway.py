...
class OpenCodeGateway:
    ...
    async def get_session_messages(self, session_id: str, limit: int = 100, before: Optional[str] = None) -> Dict:
        """获取会话消息历史"""
        if self._offline_mode:
            return self._ok({"messages": [], "session_id": session_id})
        try:
            messages, error = await self._client.get_session_messages(session_id, limit=limit, before=before)
            if error:
                return self._fail("opencode", error)
    ...
    async def get_session_children(self, session_id: str) -> Dict:
        """获取会话的子会话列表"""
        if self._offline_mode:
            return self._ok({"children": []})
        try:
            children, error = await self._client.get_session_children(session_id)
            if error:
                return self._fail("opencode", error)
            return self._ok({"children": children or []})
        except Exception as e:
            return self._fail("system", f"get_session_children 异常: {e}")
