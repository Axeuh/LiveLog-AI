...
{message}"""
    ...
    async def get_session_messages(
        ...
        try:
            ...
            logger.error(f"鑾峰彇浼氳瘽娑堟伅澶辫触: {e}")
            return None, str(e)

    async def get_session_children(self, session_id: str) -> Tuple[Optional[List[Dict]], Optional[str]]:
        """获取会话的子会话列表"""
        try:
            async with aiohttp.ClientSession(timeout=self.timeout) as client:
                async with client.get(
                    f"{self.base_url}/session/{session_id}/children"
                ) as response:
                    if response.status == 200:
                        data = await response.json()
