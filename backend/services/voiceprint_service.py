...
class VoiceprintService:
    ...
    def __init__(self):
        ...
        self._centroids: Dict[str, np.ndarray] = {}  # name -> centroid向量
        self._load_registry()
    ...
    def score_all(self, audio_data) -> Dict[str, float]:
        ...
        """
        if not self._centroids or not self._registry:
            return {}
        ...
        for name, centroid in self._centroids.items():
            sim = self.compute_similarity(embedding, centroid)
    ...
    def _rebuild_centroids(self):
        """为每个说话人重建质心向量（按 name 分组）。

        注意：调用方必须已持有 _registry_lock（enroll/delete 都持锁调用，
        避免死锁）。_load_registry 调用时需自己包装 with 锁。
        """
        self._centroids = {}
        if not self._registry:
            return
        # 按 name 分组（调用方已持锁，不在这里加锁）
        ...
        for sid, entry in self._registry.items():
            ...
            if n > 1e-8:
                groups.setdefault(name, []).append(emb / n)
        ...
        names = ", ".join(f"{n}({len(groups[n])})" for n in groups)
        logger.info(f"[Voiceprint] 声纹质心已重建: {names}")

    def _load_registry(self):
        """从 JSON 加载注册库 (v3: 含质心字典)。"""
        try:
            if os.path.exists(_VOICEPRINTS_PATH):
                ...
                if "centroids" in data and data["centroids"]:
                    ...
                    for name, vec in data["centroids"].items():
                        self._centroids[name] = np.array(vec, dtype=np.float64)
                    logger.info(f"[Voiceprint] 质心字典已加载: "
                                f"{', '.join(f'{n}({len(v)})' for n,v in data.get('centroid_info', {}).items())}")
                elif "centroid" in data and data["centroid"]:
                    # 兼容旧版 v3 单质心迁移
                    self._centroids = {}
                    name = data.get("centroid_info", {}).get("name", "unknown")
                    self._centroids[name] = np.array(data["centroid"], dtype=np.float64)
                    self._save_registry()
                else:
                    self._rebuild_centroids()
            else:
    ...
    def _save_registry(self):
        ...
        try:
            ...
            # v3: 存入质心字典
            if self._centroids:
                data["centroids"] = {name: vec.tolist() for name, vec in self._centroids.items()}
                data["centroid_info"] = {name: {"n_samples": len([e for e in self._registry.values()
                                                                     if e.get("name") == name]),
                                                 "method": "l2_norm_mean"}
