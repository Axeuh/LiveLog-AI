...
class AudioProcessor:
    ...
    def process(self, audio_path: str) -> Dict[str, Any]:
        ...
        self._calibrate_segment_confidence(segments, audio_path)
        self._enrich_segments_with_acoustic_features(segments, audio_path)
    ...
    def _enrich_segments_with_acoustic_features(self, segments: list, audio_path: str):
        """计算每个 segment 的声学特征并注入到 segment dict 中。

        计算以下特征：
        - avg_db / peak_db / db_range: 基于 RMS 的分贝值
        - zero_crossing_rate: 过零率
        - spectral_centroid: 频谱质心 (Hz)
        - voice_ratio / silence_ratio: 基于 VAD 的语音/静音比例
        """
        if not segments:
            return
        ...
        try:
            ...
            logger.warning(f"声学特征提取：无法加载音频 {audio_path}: {e}")
            return

        hop_length = 512
        fps = sr / hop_length  # 帧率 (frames per second)
        ...
        for seg in segments:
            ...
            # 帧索引：防止越界
            start_frame = min(int(seg_start * fps), n_frames - 1)
            end_frame = min(int(seg_end * fps) + 1, n_frames)
            if end_frame <= start_frame:
                end_frame = min(start_frame + 1, n_frames)

            seg_rms_db = rms_db[start_frame:end_frame]

            # 响度特征
            seg["avg_db"] = round(float(np.mean(seg_rms_db)), 2)
            seg["peak_db"] = round(float(np.max(seg_rms_db)), 2)
            seg["db_range"] = round(seg["peak_db"] - seg["avg_db"], 2)

            # ZCR
            seg["zero_crossing_rate"] = round(
                float(np.mean(zcr[start_frame:end_frame])), 4
            )

            # 频谱质心
            seg["spectral_centroid"] = round(
                float(np.mean(spectral_centroid[start_frame:end_frame])), 2
            )

            # VAD 语音/静音比例
            voice_ratio = 0.0
            silence_ratio = 0.0
            if self._last_vad_segments is not None:
                total_voice = 0.0
                for vad_seg in self._last_vad_segments:
                    vad_start = max(vad_seg["start"], seg_start)
                    vad_end = min(vad_seg["end"], seg_end)
                    if vad_end > vad_start:
                        total_voice += vad_end - vad_start
                voice_ratio = round(total_voice / seg_dur, 4)
                silence_ratio = round(1.0 - voice_ratio, 4)

            seg["voice_ratio"] = voice_ratio
            seg["silence_ratio"] = silence_ratio
