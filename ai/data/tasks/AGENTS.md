...
prompt: |-                              # 发给 AI 的提示词
  任务执行逻辑...
model: deepseek-v4-flash                # 可选，覆盖全局默认模型
```

## model 字段（可选）

不写则使用全局默认模型（当前为 `deepseek-v4-pro`）。

支持两种格式：

```yaml
# 字符串简写：只指定模型ID（使用对应默认提供商）
...
可用模型见 `GET /api/screen/model` 接口返回的列表。
...
def run(context):
    ...
    while True:
        ...
        context.heartbeat()   # 必须每300秒至少调用一次
        time.sleep(60)
```
...
| `context.getenv(key, default)`         | 读取环境变量                |
| `context.backend_info()`               | 获取后端运行信息（端口/会话ID） |
| `context.heartbeat()`                  | 保活信号（每300秒至少一次） |
...
- 允许导入：`math, json, re, datetime, time, random, typing, collections, pathlib, os, socket, http, urllib, requests`
- 禁止：`subprocess`、`shutil`、`ctypes`、`eval/exec/compile`
- 内存限制 256MB，心跳超时 300 秒，trigger_task 速率限制 1次/分钟
- 脚本可以通过 `context.backend_info()` 获取当前 OpenCode 会话ID、后端端口、OpenCode 端口
- 脚本可以读写目录（`os.listdir/mkdir/remove`）和发起网络请求（`socket/requests`）
...
name: 生产测试脚本
enabled: false
note: 完整功能演示，展示 Context API 全部能力
...
def run(context):
    ...
    context.log(f"后端端口: {info['backend_port']}, OpenCode: {info['opencode_port']}")

    while True:
        # 检查数据文件
        if os.path.isdir("ai/data"):
            today = datetime.now().strftime("%Y-%m-%d")
            data_dir = f"ai/data/{today}"
            if os.path.isdir(data_dir):
                context.log(f"今日数据目录存在: {data_dir}")
                files = os.listdir(data_dir)
                context.log(f"文件数: {len(files)}")
            else:
                context.log("今日暂无数据")
