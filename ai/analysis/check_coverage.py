...
def get_understood_last_time(date_str, base_dir='data'):
    ...
    # 取最晚时段（理解文件可能非严格时序追加，不能用 matches[-1]）
    def _parse_time_safe(t_str):
        val = parse_time(t_str)
        return val if val is not None else -1

    last_start_str, last_end_str = max(matches, key=lambda m: _parse_time_safe(m[1]))
