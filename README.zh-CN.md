...
> **现状说明**：本项目处于初期阶段。作者忙于学业，但系统本人一直在使用，运行稳定。可能存在历史遗留问题和兼容性缺陷。目前仅在 Windows 平台测试，Linux 和 macOS 理论上可以运行但未经测试。前端（Vue SPA）包含测试沙箱 `frontend/refactor/`（子智能体嵌套、Token 用量显示等功能在此验证），稳定后同步到生产目录 `frontend/mobile/`。
>
...
启动后，AI 系统**全自动运行**——无需人工介入。它自动采集数据、自动处理、自动生成报告、自动管理记忆、自动推送洞察到手机。整条流水线每一步都是自动化的。
...
graph LR
    subgraph "脚本层 - 数据预处理"
        FE[fact_extractor.py<br/>原始 JSON 转可读 Markdown]
        CC[check_coverage.py<br/>检查时间覆盖完整性]
    end
    subgraph "子智能体层 - 专业分析"
        DC[data-check 智能体<br/>写入理解记录]
        DR[daily-review 智能体<br/>生成复盘报告]
    end
    subgraph "主智能体层 - 协调验收"
...
- **脚本层** 自动运行：原始数据 → 可读格式 → 完整性验证，全程无需干预。
- **子智能体层** 自动派生：专门智能体自动写理解记录、生成报告、收割记忆。
- **主智能体层** 自动协调：验收输出、发现异常自动重试、随时间自动优化工作流。

#### 17 个自动化任务

系统全天候运行一套自动化任务链。每个任务都是一个完整的自主流水线——自动触发、自动处理、自动分析、自动提交：

| 任务组 | 数量 | 周期 | 自动化范围 |
|--------|------|------|-----------|
| 数据检查 | 12 | 每 2 小时 | 自动预处理 → 自动分析 → 自动验证 → 自动提交 |
| 每日复盘 | 1 | 每天 | 自动读取 → 自动交叉检查 → 自动报告 → 自动记忆收割 → 自动通知 |
| 双周回顾 | 1 | 每月 1/16 日 | 自动扫描 14 天 → 自动模式分析 → 自动报告 |
| 每周总结 | 1 | 每周日 | 自动编译 7 天 → 自动叙事 → 自动报告 |
| 学习助手 | 3 | 工作日早/中/晚 | 自动跟踪 → 自动鼓励 → 自动通知 |

每个数据检查任务都是一条完整的自动化流水线：`fact_extractor.py` 自动预处理原始数据 → `01-data-check` 子智能体自动写理解记录 → `check_coverage.py` 自动验证完整性 → git 自动提交——全过程无人介入。

#### 6 个子智能体

每个子智能体有独立的 prompt 文件（`ai/agents/*.md`），精确规定了输入、输出和约束：

| 智能体 | 用途 | 输出 |
|--------|------|------|
| **01-data-check** | 数据检查 | 写入 `{date}-理解.md` |
| **02-daily-review** | 每日复盘 | 生成 `复盘-{date}.md` + 记忆收割 |
| **03-biweekly-review** | 深度回顾 | 跨日行为模式洞察 |
| **04-weekly-review** | 周报总结 | 叙事性一周记录 |
| **05-feedback-agent** | 反馈推送 | 提取关注点、匹配偏好推送 |
| **06-study-coach** | 学习助手 | 跟踪学习进度 + 陪伴式通知 |

#### Python 自定义脚本 — 超越定时调度

除了 17 个 YAML 定时任务，你可以在 `ai/data/tasks/` 下编写 Python 脚本，作为长期循环运行，自由控制频率、条件和外部集成：
...
def run(context):
    while True:
        data = context.read_file("ai/data/2026-06-25/perception.jsonl")
        if data and 条件满足:
...
| 能力 | 示例 |
|------|------|
| 自定义调度 | 只在用户清醒时段检查，夜间降低频率 |
| 条件触发 | 只有新的语音事件到达才触发 AI |
| 外部 API 集成 | 拉取天气数据、智能家居设备状态 |
| 智能家居联动 | GPS 到家触发到家场景、检测到睡眠触发夜间模式 |
| 跨脚本通信 | 多个脚本通过 `ai/data/` 下的文件共享状态 |
| 后端 API 调用 | 通过 localhost 触发 TTS 播报、推送通知 |

脚本运行在沙箱中（256 MB 内存、AST 安全检查、心跳保活），通过 `context` API 记录日志、读取文件、触发 AI 任务和报告错误。详见 `docs/AI_SYSTEM.md`。

#### 长期记忆系统

AI 维护自己的长期记忆，存储在 `ai/记忆/` 目录下。每日复盘时自动收割：

- **新地点**：GPS 坐标或地名首次出现
- **新偏好**：饮食、娱乐、作息习惯首次或持续出现
- **新社交关系**：对话中出现的新人名
- **健康基线变化**：持续的生理指标变化（非临时波动）
- **经验教训**：可复用的踩坑或成功记录
...
- **数据优先**：每条分析结论必须引用原始数据。没有源数据的分析无效。
- **诚实标注**：每项判断标注置信度（确定/高/中/低/数据不足）。
- **子智能体研究，主智能体决策**：子智能体做专业分析，主智能体做协调和决策。
- **反馈回路**：每次任务完成后推送手机通知。异常和疑惑主动推送，不等用户问。
- **边界意识**：AI 限定在 `ai/` 目录内工作——读感知数据、写分析文件，不碰项目源代码。

> 完整 AI 系统架构文档：[docs/AI_SYSTEM.md](docs/AI_SYSTEM.md)
...
> 完整数据格式文档见 [docs/DATA_FORMAT.md](docs/DATA_FORMAT.md)
