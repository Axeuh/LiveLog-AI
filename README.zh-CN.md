...
后端调度器每秒轮询，到达预定时间触发 AI 任务：

| 任务组 | 数量 | 频率 | 用途 |
|--------|------|------|------|
| 数据检查 | 12 | 每 2 小时 | 每次检查 2 小时窗口，保证分析实时性 |
| 每日复盘 | 1 | 每天 02:00 | 全天回顾报告 + 长期记忆收割 |
| 双周回顾 | 1 | 每月 1/16 日 | 跨天行为模式深度分析 |
| 每周总结 | 1 | 每周日 | 叙事性周报 |
| 学习助手 | 3 | 工作日早/中/晚 | 学习进度跟踪和提醒 |

每个数据检查任务的流程完全相同：`fact_extractor.py` 预处理 → `01-data-check` 子智能体写理解记录 → `check_coverage.py` 验证无时间缺口 → git 提交。
