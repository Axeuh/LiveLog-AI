...
├── frontend/mobile/   # WebView SPA 前端 生产 (Vue 3/TS) - 见 frontend/mobile/AGENTS.md
├── frontend/refactor/ # WebView SPA 前端 测试沙箱 (Vue 3/TS) - Apple Health 深色设计系统 + 子智能体 - 见 frontend/refactor/AGENTS.md
...
| 前端页面/组件 | `frontend/refactor/src/views/` + `components/` (开发), `frontend/mobile/` (生产) |
| 前端组合式函数 | `frontend/refactor/src/composables/` (开发,19个), `frontend/mobile/` (生产,11个) |
| 子智能体/嵌套会话 | `frontend/refactor/src/composables/useSubAgents.ts` + `ChatView.vue` |
| Token用量显示 | `frontend/refactor/src/composables/useStreamingMessage.ts` + `useChat.ts` |
...
### TypeScript/Vue 约定 (frontend/)
- Vue 3 Composition API + `<script setup>`，严格TS模式
- 无 Pinia —— 所有状态用 composables 管理，参考 `frontend/refactor/src/composables/`
- Router hash 模式，路径别名 `@` -> `./src`
- 测试沙箱 `refactor/` 与生产 `mobile/` 独立迭代，稳定后手动同步
- 集中式Store模式: SSE事件直接修改 `messages.value`，不依赖占位符注册
- 子智能体系统: 多级嵌套栈 (`parentChain`)，API + 消息扫描双重数据源
- Token用量: 双路径更新 (SSE实时 + API历史加载)
...
- `frontend/refactor/` (测试沙箱) 使用 Apple Health 深色设计系统，与 `frontend/mobile/` (生产) 独立迭代，稳定后手动同步。详见 `frontend/refactor/AGENTS.md`
- 子智能体支持多级嵌套 (parentChain 栈)，任何层级可继续打开子智能体列表
- 会话 Token 用量显示: 双路径更新 (SSE `info.tokens` 实时 + API 消息历史加载)
- `USE_MOCK_DATA` 开关位于 `frontend/refactor/src/composables/useMockData.ts`，生产部署时改为 `false`
