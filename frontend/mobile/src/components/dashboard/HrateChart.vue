<template>
  <div class="hrate-chart" :class="{ expanded: isExpanded }">
 ...
 *
 * 展示心率数据的时间序列折线图
 * - 颜色: #ff6b6b, 填充: rgba(255,107,107,0.08)
 * - Y轴范围: 40-110 bpm
 * - 降采样: 最多 80 个数据点
 * - 响应式布局
 */
...
const timeLabels = computed(() =>
  generateTimeLabels(props.samples.filter((s) => s.hr != null)),
);

/** 降采样后的数据 */
const chartData = computed(() =>
  downsample(hrData.value, timeLabels.value, 80),
);
