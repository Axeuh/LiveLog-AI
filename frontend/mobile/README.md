...
│   │   ├── useLayout.ts               # PC/移动端布局检测
│   │   ├── useMockData.ts             # Mock 数据全局开关
│   │   ├── usePrefixConfig.ts         # 前缀配置检测
│   │   ├── useRealTime.ts             # 实时连接管理 (SSE + WS)
│   │   ├── useReorder.ts              # 拖拽排序
│   │   ├── useReports.ts              # 报告数据管理
│   │   ├── useSendMessage.ts          # 消息发送逻辑
│   │   ├── useStreamingMessage.ts     # 流式消息处理 (集中式 Store)
│   │   ├── useSubAgents.ts            # 子智能体 (多级嵌套栈)
│   │   ├── useTheme.ts                # 主题管理 (dark/light/glacier)
│   │   └── useTtsPlayer.ts            # TTS 语音播报
│   │
...
- **`useChat`** -- 管理聊天消息列表、会话切换、消息历史、Token 用量提取
- **`useStreamingMessage`** -- 处理流式消息的逐 token 渲染（集中式 Store 模式，SSE 直接修改消息 parts）
- **`useSendMessage`** -- 封装消息发送请求与重试逻辑
- **`useSubAgents`** -- 子智能体管理（多级嵌套栈，API + 消息扫描双重数据源）
